...
[[vk::binding(4, 0)]]
RWTexture2D<float4>         g_outputDbgImage    ;
#endif
...
void XeGTAO_PrefilterDepths16x16( uint2 dispatchThreadID /*: SV_DispatchThreadID*/, uint2 groupThreadID /*: SV_GroupThreadID*/, const GTAOConstants consts, Texture2D<float> sourceNDCDepth, SamplerState depthSampler, RWTexture2D<lpfloat> outDepth0, RWTexture2D<lpfloat> outDepth1, RWTexture2D<lpfloat> outDepth2, RWTexture2D<lpfloat> outDepth3, RWTexture2D<lpfloat> outDepth4 )
{
    ...
    #ifdef XE_GTAO_VIEWSPACE_DEPTH
        lpfloat depth0 = XeGTAO_ClampDepth( depths4.w );
        lpfloat depth1 = XeGTAO_ClampDepth( depths4.z );
        lpfloat depth2 = XeGTAO_ClampDepth( depths4.x );
        lpfloat depth3 = XeGTAO_ClampDepth( depths4.y );
    #else
...
void XeGTAO_Denoise( const uint2 pixCoordBase, const GTAOConstants consts, Texture2D<uint> sourceAOTerm, Texture2D<lpfloat> sourceEdges, SamplerState texSampler0, SamplerState texSampler1, RWTexture2D<uint> outputTexture, const uniform bool finalApply )
{
    ...
    lpfloat4 edgesQ0        = sourceEdges.GatherRed( texSampler1, gatherCenter, int2( 0, 0 ) );
    lpfloat4 edgesQ1        = sourceEdges.GatherRed( texSampler1, gatherCenter, int2( 2, 0 ) );
    lpfloat4 edgesQ2        = sourceEdges.GatherRed( texSampler1, gatherCenter, int2( 1, 2 ) );

    AOTermType visQ0[4];    XeGTAO_DecodeGatherPartial( sourceAOTerm.GatherRed( texSampler0, gatherCenter, int2( 0, 0 ) ), visQ0 );
    AOTermType visQ1[4];    XeGTAO_DecodeGatherPartial( sourceAOTerm.GatherRed( texSampler0, gatherCenter, int2( 2, 0 ) ), visQ1 );
    AOTermType visQ2[4];    XeGTAO_DecodeGatherPartial( sourceAOTerm.GatherRed( texSampler0, gatherCenter, int2( 0, 2 ) ), visQ2 );
    AOTermType visQ3[4];    XeGTAO_DecodeGatherPartial( sourceAOTerm.GatherRed( texSampler0, gatherCenter, int2( 2, 2 ) ), visQ3 );
